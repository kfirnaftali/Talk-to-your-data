...
class GemmaModelHandler(ModelHandler[str, PredictionResult, Gemma3CausalLM]):
  """
  A RunInference model handler for the Gemma model using keras_hub.
  """

  def __init__(self,
               model_name: str = "gemma3_instruct_4b",
               max_length: int = 64):
    ...
    self._env_vars = {}
    logging.info(
        f"Initializing GemmaModelHandler (keras_hub) with model: {self._model_name}, max_length: {self._max_length}"
    )

  def share_model_across_processes(self) -> bool:
    return True

  def load_model(self) -> Gemma3CausalLM:
    if self._model is None:
      logging.info(f"Loading keras_hub model: {self._model_name}")
      try:
        self._model = keras_hub.models.Gemma3CausalLM.from_preset(
            self._model_name)
        logging.info(f"keras_hub model {self._model_name} loaded successfully.")
      except Exception as e:
        logging.exception(
            f"Failed to load keras_hub model {self._model_name}: {e}")
        raise
    return self._model
  ...
  ) -> Iterable[PredictionResult]:  # Changed unused to inference_args
    logging.debug(
        f"Running keras_hub inference: batch_size={len(batch)}, max_length={self._max_length}"
    )
    results = []
    try:
      ...
      results = [
          PredictionResult(
              example=prompt,
              inference=f"Inference Error: {e}",
              model_id=self._model_name) for prompt in batch
      ]
    return results
